    ...
    function login()
    {
        if (! $this->session->userdata('DX_logged_in')) {
            if (!empty($_POST['email']) && !empty($_POST['password'])) {
                ...
                $row = $this->user->get_single_entry_by_email($_POST['email']);
                ...
                if (crypt($password, $stored_hash) === $stored_hash) {
                    ...
                    echo alert('danger', '', 'Password or username is incorrect1');
                }
            } else{
                echo alert('danger', '', 'Password or username are empty3');
            }
        } else{
            redirect('/home');
        }
