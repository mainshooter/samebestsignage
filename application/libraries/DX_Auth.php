...
class DX_Auth
{
	...
	*/
	...
	{
		$majorsalt = (!empty($this->ci->config->item('DX_salt')))? $this->ci->config->item('DX_salt'): 'UITY&O*7d8u09pasolkJGDT))polkhjg879SOI';
	...
	*/
	...
	function login($login, $password, $remember = TRUE)
	{
		...
		if ( ! empty($login) AND ! empty($password))
		{
			...
			if ($query = $this->ci->users->$get_user_function($login) AND $query->num_rows() == 1)
			{
				...
				{					
					...
					else						
					{
						...
						// Set error message
						$this->_auth_error = alert('danger', '', 'Password incorrect!!!');
					}
				}				
			}
			...
			else
			{
				...
				$this->_increase_login_attempt();
				// Set error message
				$this->_auth_error = alert('danger', '', 'Username incorrect!!!');
			}
		}
	...
	function register($username, $password, $email, $role = 1)
	{		
		// Load Models
		$this->ci->load->model('dx_auth/users', 'users');
		$this->ci->load->model('dx_auth/user_temp', 'user_temp');
	...
	function change_password_user($id, $new_pass)
	{
		// Load Models
		$this->ci->load->model('dx_auth/users', 'users');
		...
		// Search current logged in user in database
		if ($query = $this->ci->users->get_user_by_id($id) AND $query->num_rows() > 0)
		{
			// Get current logged in user
			$row = $query->row();
			...
			// Trigger event
			$this->ci->dx_auth_event->user_changed_password($id, $new_pass);
